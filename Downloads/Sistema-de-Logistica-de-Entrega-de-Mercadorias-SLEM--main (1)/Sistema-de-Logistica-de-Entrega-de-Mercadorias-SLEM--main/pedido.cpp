...
void Pedido::cadastroPedido()
{
    ...
    cout << endl
         << "Insira o endereco de origem:" << endl;
    cin.ignore();
    ...
    cout << endl
         << "Insira o endereco de destino:" << endl;
    ...
    cout << endl
         << "Insira o peso do item (em kg):" << endl;
    cin >> peso;
...
void Pedido::listarPedidos()
{
    ...
    {
        cout << endl
             << "ID: " << pedido.getIdentificadorUnico() << endl;
...
// Função para salvar pedidos em arquivo binário
void Pedido::salvarPedidosEmArquivoBinario()
{
    ofstream arquivo("pedidos.dat", ios::binary);
    if (!arquivo)
    {
        cout << "Erro ao abrir arquivo de backup de pedidos.\n";
        return;
    }
    ...
    {
        PedidoSerializado p;
        p.id = pedido.identificadorUnico;
        strcpy(p.nomeOrigem, pedido.getLocalOrigem()->getNomeLocal());
        strcpy(p.nomeDestino, pedido.getLocalDestino()->getNomeLocal());
        p.peso = pedido.getPesoItem();
        p.pendente = pedido.getPendente();

        arquivo.write((char *)&p, sizeof(PedidoSerializado));
    }

    arquivo.close();
    cout << "Backup de pedidos concluído.\n";
}

// Função para carregar pedidos de arquivo binário
void Pedido::carregarPedidosDeArquivoBinario()
{
    ifstream arquivo("pedidos.dat", ios::binary);
    if (!arquivo)
    {
        cout << "Nenhum arquivo de backup de pedidos encontrado.\n";
        return;
    }
    ...
    while (arquivo.read((char *)&p, sizeof(PedidoSerializado)))
    {
        Local *origem = encontrarLocalPorNome(p.nomeOrigem);
        Local *destino = encontrarLocalPorNome(p.nomeDestino);

        if (origem && destino)
        {
            Pedido novo(origem, destino, p.peso);
            novo.setPendente(p.pendente);
            novo.identificadorUnico = p.id;
    ...
    arquivo.close();
    cout << "Restauração de pedidos concluída.\n";
}
